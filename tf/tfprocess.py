...
class ApplySqueezeExcitation(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        x = inputs[0]
        excited = inputs[1]
        gammas, betas = tf.split(tf.reshape(
            excited, [-1, self.reshape_size, 1, 1]), 2, axis=1)
...
class TFProcess:
    def __init__(self, cfg):
        ...
        if self.cfg['gpu'] == 'all':
            ...
            tf.config.experimental.set_visible_devices(
                gpus[self.cfg['gpu']], 'GPU')
            tf.config.experimental.set_memory_growth(
                gpus[self.cfg['gpu']], True)
            self.strategy = None
    ...
    def init_net_v2(self):
        ...
        if self.swa_enabled:
            ...
            self.swa_weights = [tf.Variable(
                w, trainable=False) for w in self.model.weights]
        ...
        self.checkpoint = tf.train.Checkpoint(
            optimizer=self.orig_optimizer, model=self.model, global_step=self.global_step, swa_count=self.swa_count)
        ...
        for e, weights in enumerate(self.model.weights):
            ...
            if weights.shape.ndims == 4:
                ...
                new_weight = tf.constant(
                    new_weights[source_idx], shape=weights.shape)
    ...
    def process_loop_v2(self, batch_size, test_batches, batch_splits=1):
        ...
        for _ in range(steps % total_steps, total_steps):
            self.process_v2(batch_size, test_batches,
    ...
    def strategy_process_inner_loop(self, x, y, z, q):
        policy_loss, value_loss, mse_loss, reg_term, new_grads = self.strategy.experimental_run_v2(
    ...
    def apply_grads(self, grads, batch_splits):
        ...
        max_grad_norm = self.cfg['training'].get(
            'max_grad_norm', 10000.0) * batch_splits
    ...
    def strategy_apply_grads(self, grads, batch_splits):
        ...
        grads = [self.strategy.reduce(
            tf.distribute.ReduceOp.MEAN, x, axis=None) for x in grads]
    ...
    def process_v2(self, batch_size, test_batches, batch_splits=1):
        ...
        if self.warmup_steps > 0 and steps < self.warmup_steps:
            self.lr = self.lr * \
                tf.cast(steps + 1, tf.float32) / self.warmup_steps
        ...
        for _ in range(batch_splits):
            ...
            if self.strategy is not None:
                policy_loss, value_loss, mse_loss, reg_term, new_grads = self.strategy_process_inner_loop(
                    x, y, z, q)
            else:
        ...
        if steps % self.cfg['training']['train_avg_report_steps'] == 0 or steps % self.cfg['training']['total_steps'] == 0:
            ...
            if self.time_start:
                ...
                speed = batch_size * \
                    (tf.cast(steps_elapsed, tf.float32) / elapsed)
    ...
    def strategy_calculate_test_summaries_inner_loop(self, x, y, z, q):
        policy_loss, value_loss, mse_loss, policy_accuracy, value_accuracy = self.strategy.experimental_run_v2(
    ...
    def calculate_test_summaries_v2(self, test_batches, steps):
        ...
        for _ in range(0, test_batches):
            ...
            if self.strategy is not None:
                policy_loss, value_loss, mse_loss, policy_accuracy, value_accuracy = self.strategy_calculate_test_summaries_inner_loop(
                    x, y, z, q)
            else:
        ...
        with self.test_writer.as_default():
            ...
            tf.summary.scalar("Policy Accuracy",
                              sum_policy_accuracy, step=steps)
            if self.wdl:
                tf.summary.scalar("Value Accuracy",
                                  sum_value_accuracy, step=steps)
    ...
    def compute_update_ratio_v2(self, before_weights, after_weights, steps):
        ...
        weight_norms = [tf.math.reduce_euclidean_norm(
            w) for w in before_weights]
        ratios = [(tensor.name, tf.cond(w != 0., lambda: d / w, lambda: -1.)) for d, w,
                  tensor in zip(delta_norms, weight_norms, self.model.weights) if not 'moving' in tensor.name]
        ...
        ratios = [tf.cond(r > 0, lambda: tf.math.log(
            r) / 2.30258509299, lambda: 200.) for (_, r) in ratios]
        tf.summary.histogram('update_ratios_log10', tf.stack(
            ratios), buckets=1000, step=steps)

    def update_swa_v2(self):
        num = self.swa_count.read_value()
        for (w, swa) in zip(self.model.weights, self.swa_weights):
            swa.assign(swa.read_value() * (num / (num + 1.)) +
                       w.read_value() * (1. / (num + 1.)))
    ...
    def squeeze_excitation_v2(self, inputs, channels):
        ...
        squeezed = tf.keras.layers.Activation('relu')(tf.keras.layers.Dense(
            channels // self.SE_ratio, kernel_initializer='glorot_normal', kernel_regularizer=self.l2reg)(pooled))
        excited = tf.keras.layers.Dense(
            2 * channels, kernel_initializer='glorot_normal', kernel_regularizer=self.l2reg)(squeezed)
        return ApplySqueezeExcitation()([inputs, excited])

    def conv_block_v2(self, inputs, filter_size, output_channels, bn_scale=False):
        conv = tf.keras.layers.Conv2D(output_channels, filter_size, use_bias=False, padding='same',
                                      kernel_initializer='glorot_normal', kernel_regularizer=self.l2reg, data_format='channels_first')(inputs)
        return tf.keras.layers.Activation('relu')(self.batch_norm_v2(conv, scale=bn_scale))

    def residual_block_v2(self, inputs, channels):
        conv1 = tf.keras.layers.Conv2D(channels, 3, use_bias=False, padding='same', kernel_initializer='glorot_normal',
                                       kernel_regularizer=self.l2reg, data_format='channels_first')(inputs)
        out1 = tf.keras.layers.Activation('relu')(
            self.batch_norm_v2(conv1, scale=False))
        conv2 = tf.keras.layers.Conv2D(channels, 3, use_bias=False, padding='same', kernel_initializer='glorot_normal',
                                       kernel_regularizer=self.l2reg, data_format='channels_first')(out1)
    ...
    def construct_net_v2(self, inputs):
        ...
        if self.POLICY_HEAD == pb.NetworkFormat.POLICY_CONVOLUTION:
            ...
            conv_pol2 = tf.keras.layers.Conv2D(80, 3, use_bias=True, padding='same', kernel_initializer='glorot_normal',
                                               kernel_regularizer=self.l2reg, bias_regularizer=self.l2reg, data_format='channels_first')(conv_pol)
            ...
            h_fc1 = tf.keras.layers.Dense(1858, kernel_initializer='glorot_normal',
                                          kernel_regularizer=self.l2reg, bias_regularizer=self.l2reg)(h_conv_pol_flat)
        else:
            raise ValueError(
        ...
        h_fc2 = tf.keras.layers.Dense(128, kernel_initializer='glorot_normal',
                                      kernel_regularizer=self.l2reg, activation='relu')(h_conv_val_flat)
        if self.wdl:
            h_fc3 = tf.keras.layers.Dense(3, kernel_initializer='glorot_normal',
                                          kernel_regularizer=self.l2reg, bias_regularizer=self.l2reg)(h_fc2)
        else:
            h_fc3 = tf.keras.layers.Dense(
                1, kernel_initializer='glorot_normal', kernel_regularizer=self.l2reg, activation='tanh')(h_fc2)
        return h_fc1, h_fc3
